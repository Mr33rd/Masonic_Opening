...
// ── Door close: wooden thud + impact crack ───────────────────────────────────
function doorClose() {
  const ctx = getCtx()
  const now = ctx.currentTime

  // Low body resonance — decaying sine sweep
...
// ── Lock click: mechanical snap + metallic ping ──────────────────────────────
function lockClick() {
  const ctx = getCtx()
  const now = ctx.currentTime

  // Mechanical snap (high-passed noise burst)
